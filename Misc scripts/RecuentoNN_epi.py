...
wd = r'C:\Users\Usuario\Desktop\Joaquin\Test\20250602 - Tul y Dm wt 43a1'
os.chdir(wd)
...
main_folder = r'C:\Users\Usuario\Desktop\Joaquin\Test\20250602 - Tul y Dm wt 43a1\Fotos'

from os.path import isfile, join 

files =  [f for f in os.listdir(main_folder) if isfile(join(main_folder, f))] # Makes a list of the names of all the images in the folder
...
for l in field_identifiers:
    groups.setdefault(l[-0], []).append(l) #Change l[-x] to whatever position holds the field number
...
for fov in range(len(fields)):
    ...
    campo = os.path.join(main_folder, fields[fov][0])
    img = imread(campo) 
    imgs.append(img)
...
for im in range(len(imgs)):  
    ...
    identifiers = fields[im][0].split("-")
    no_channel = identifiers[2].split("_")
    conditions = [identifiers[0], identifiers[1], no_channel[0]] 
    ...
    #Plot
    fig_name = "%s.png" % (fields[im][0])  
    fig = summary_plot(fields[im][0], imgs[im], masks_Tc[im], masks_Nuc[im], pairs, percent_infected, amastigotes_per_infected, amastigotes_in_cell)
